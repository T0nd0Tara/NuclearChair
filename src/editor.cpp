...
private:
  ...
  void handleControls() {
    ...
    if (GetKey(olc::Key::RIGHT).bHeld)
      tv.MoveWorldOffset(olc::vf2d{BLOCK_SIZE, 0} * PAN_SPEED *
                         GetElapsedTime() / tv.GetWorldScale().x);
    if (GetKey(olc::Key::LEFT).bHeld)
      tv.MoveWorldOffset(olc::vf2d{-BLOCK_SIZE, 0} * PAN_SPEED *
                         GetElapsedTime() / tv.GetWorldScale().x);
    if (GetKey(olc::Key::DOWN).bHeld)
      tv.MoveWorldOffset(olc::vf2d{0, BLOCK_SIZE} * PAN_SPEED *
                         GetElapsedTime() / tv.GetWorldScale().x);
    ...
    if (nCurrDecal < 0)
      nCurrDecal = vDecals[nCurrType].size() - 1;
    else
      nCurrDecal = nCurrDecal % vDecals[nCurrType].size();
    ...
    if (GetKey(olc::Key::ENTER).bHeld) {
      ...
      case (int)WT::MOBS: {
        auto it =
            std::find_if(vMobs.begin(), vMobs.end(), [this](const auto &mob) {
              return vCurrCell == mob.pos;
            });
        if (it != vMobs.end())
          vMobs.erase(it);
    ...
    // fill area
    if (GetKey(olc::Key::F).bPressed && nCurrType == (int)WT::BLOCKS) {
  ...
  void tvDrawRectDecal(olc::vf2d pos, olc::vf2d size,
...
public:
  ...
  bool OnUserUpdate(float fElapsedTime) override {
    ...
    // Draw Grid
    if (bDrawGrid) {
      for (float x = float(TL.x / BLOCK_SIZE) * BLOCK_SIZE; x < BR.x;
           x += BLOCK_SIZE)
        if (x != 0.0f)
          tv.DrawLine({x, (float)TL.y}, {x, (float)BR.y}, olc::GREY);

      for (float y = float(TL.y / BLOCK_SIZE) * BLOCK_SIZE; y < BR.y;
           y += BLOCK_SIZE)
        if (y != 0.0f)
    ...
    for (int y = TL.y / BLOCK_SIZE; y <= BR.y / BLOCK_SIZE; y++)
      for (int x = TL.x / BLOCK_SIZE; x <= BR.x / BLOCK_SIZE; x++) {
        ...
        tv.DrawDecal(olc::vf2d{(float)x, (float)y} * BLOCK_SIZE,
                     vDecals[(int)WT::BLOCKS][block.nDecal],
    ...
    for (const auto &mob : vMobs) {
      ...
      tv.DrawPartialDecal(mob.pos * BLOCK_SIZE, BLOCK_SIZE * relSize,
                          vDecals[(int)WT::MOBS][mob.nDecal], olc::vf2d{0, 0},
                          MOB_SIZE);
    }
    ...
    if (player.nDecal == 1)
      tv.DrawPartialDecal(
          player.pos * BLOCK_SIZE, vDecals[(int)WT::MOBS][player.nDecal],
          olc::vf2d{0, 0}, MOB_SIZE, BLOCK_SIZE * BLOCK_SIZE / MOB_SIZE);

    if (vCurrCell.x < TL.x / BLOCK_SIZE)
      vCurrCell.x = TL.x / BLOCK_SIZE;
    if (vCurrCell.y < TL.y / BLOCK_SIZE)
      vCurrCell.y = TL.y / BLOCK_SIZE;
    if (vCurrCell.x > BR.x / BLOCK_SIZE)
      vCurrCell.x = BR.x / BLOCK_SIZE;
    if (vCurrCell.y > BR.y / BLOCK_SIZE)
      vCurrCell.y = BR.y / BLOCK_SIZE;

    // 2 times for contrust
    tvDrawRectDecal(olc::vf2d{(float)vCurrCell.x, (float)vCurrCell.y} *
                            BLOCK_SIZE +
                        olc::vf2d{0.1f, 0.1f},
                    {(float)BLOCK_SIZE, BLOCK_SIZE}, olc::BLACK);
    tvDrawRectDecal(olc::vf2d{(float)vCurrCell.x, (float)vCurrCell.y} *
                        BLOCK_SIZE,
                    {(float)BLOCK_SIZE, BLOCK_SIZE});

    // Drawing World Type
    DrawStringDecal(olc::vf2d{1.0f, 1.0f},
                    std::string(magic_enum::enum_name<WT>((WT)nCurrType)),
                    olc::WHITE, olc::vf2d{1.2f, 1.2f});
    // Drawing BG Mode Label
    if (bBGMode)
      DrawStringDecal(olc::vf2d{1.0f, 16.0f}, "BG MODE");

    // Drawing Preview Block
    int prevSize = BLOCK_SIZE / 2;
    olc::vi2d vBlockPos = olc::vi2d{ScreenWidth(), ScreenHeight()} -
                          olc::vi2d{prevSize + 1, prevSize + 1};
    const std::array<olc::vf2d, 4> pos{
        vBlockPos, vBlockPos + olc::vi2d{0, prevSize},
        vBlockPos + olc::vi2d{prevSize, prevSize},
        vBlockPos + olc::vi2d{prevSize, 0}};
    switch (nCurrType) {
    case (int)WT::BLOCKS:
    ...
    case (int)WT::MOBS:
      if (vDecals[nCurrType][nCurrDecal])
        DrawPartialWarpedDecal(vDecals[nCurrType][nCurrDecal], pos,
  ...
  bool OnConsoleCommand(const std::string &sText) override {
    ...
    if (c1 == "grid") {
      ...
      if (c2 == "off" || c2 == "0") {
        ...
        std::cout << "Unknown subcommand '" << c2 << "' for command 'grid'\n";
      return true;
    }
    if (c1 == "save") {
      std::string c2;
