...
public:
  NuclearChair() { sAppName = "NuclearChair"; }
...
private:
  ...
  struct Bullet {
    ...
    inline void move(float fElapsedTime) {
      if (nDecal == -1)
        return;
      pos += olc::vf2d{cosf(fDir), sinf(fDir)} * vBulletSpeed.at(nDecal) *
             fElapsedTime;
    }
  };
  ...
  void handleControls() {
    ...
    if (GetKey(olc::Key::MINUS).bPressed)
      tv.ZoomAtScreenPos(0.5f, olc::vi2d{ScreenWidth(), ScreenHeight()} / 2);
    if (GetKey(olc::Key::EQUALS).bPressed)
      tv.ZoomAtScreenPos(2.0f, olc::vi2d{ScreenWidth(), ScreenHeight()} / 2);
    ...
    if (player.nDecal == 1) {
      ...
      if (mag > 0.0f) {
        ...
        {
          if (!map.get(newPos().x, player.pos.y).bBG ||
              !map.get(newPos().x, player.pos.y + fOpPadding).bBG)
            diffPos.x = 0.0f;
        } else if (diffPos.x > 0.0f) // moving right
        {
          if (!map.get(newPos().x + 1.0f, player.pos.y).bBG ||
              !map.get(newPos().x + 1.0f, player.pos.y + fOpPadding).bBG)
            diffPos.x = 0.0f;
        }
        if (diffPos.y < 0.0f) // moving up
        {
          if (!map.get(player.pos.x, newPos().y).bBG ||
              !map.get(player.pos.x + fOpPadding, newPos().y).bBG)
            diffPos.y = 0.0f;
        } else if (diffPos.y > 0.0f) // moving down
        {
          if (!map.get(player.pos.x, newPos().y + 1.0f).bBG ||
              !map.get(player.pos.x + fOpPadding, newPos().y + 1.0f).bBG)
            diffPos.y = 0.0f;
        }
        player.pos = newPos();
      }
      ...
      if (GetMouse(0).bPressed) {
        const olc::vf2d vDiff =
            target.pos - (player.pos + olc::vf2d{0.5f, 0.5f});
        const float fAngle = std::atan2(vDiff.y, vDiff.x);
        vBullets.emplace_back(1, player.pos + olc::vf2d{0.5f, 0.5f}, fAngle,
                              false, true);
      }
    }
  }
  ...
  void updateGame() {
    ...
    tv.SetWorldOffset(lerp(player.pos, target.pos, fPlayerPerc) * fBLOCK_SIZE -
                      tv.GetWorldVisibleArea() / 2.0f);
    ...
    for (int i = 0; i < vBullets.size(); i++) {
      ...
      if (b.bDamageEnemies) {
        for (int nMob = 0; nMob < vMobs.size(); nMob++) {
          auto &m = vMobs[nMob];
          if (rectsOverlapping(m.pos * fBLOCK_SIZE, vBLOCK_SIZE,
                               b.pos * fBLOCK_SIZE, vBULLET_SIZE)) {
            m.nHp -= b.getDamage();
  ...
  template <typename T> static inline T lerp(const T &a, const T &b, float t) {
    return a + (b - a) * t;
  }

  static olc::Pixel pixelLerp(const olc::Pixel &a, const olc::Pixel &b,
                              float t) {
    int32_t ar = a.r * a.r;
    int32_t ag = a.g * a.g;
    int32_t ab = a.b * a.b;
    ...
    return olc::Pixel(std::sqrt(r_), std::sqrt(g_), std::sqrt(b_),
                      std::sqrt(a_));
  }
  ...
  static inline bool pointInRect(olc::vf2d rectPos, olc::vf2d rectSize,
                                 olc::vf2d point) {
    return (rectPos.x <= point.x) && (point.x < rectPos.x + rectSize.x) &&
           (rectPos.y <= point.y) && (point.y < rectPos.y + rectSize.y);
  }
  static bool rectsOverlapping(olc::vf2d a_pos, olc::vf2d a_size,
                               olc::vf2d b_pos, olc::vf2d b_size) {
...
public:
  bool OnUserCreate() override {
    ...
    for (int nType = 0; nType < magic_enum::enum_count<WT>(); nType++) {
      ...
      for (const auto &file :
           std::filesystem::directory_iterator("world_types/" + sDir))
      ...
      for (const auto &path : files) {
        ...
        if (sPath.size() <= 4)
          continue;
        if (sPath.substr(sPath.size() - 4, 4) != ".png")
          continue;
  ...
  bool OnUserUpdate(float fElapsedTime) override {
    ...
    for (int y = TL.y / BLOCK_SIZE; y <= BR.y / BLOCK_SIZE; y++)
      for (int x = TL.x / BLOCK_SIZE; x <= BR.x / BLOCK_SIZE; x++) {
        ...
        tv.DrawDecal(olc::vf2d{(float)x, (float)y} * BLOCK_SIZE,
                     vWTDecals[(int)WT::BLOCKS][block.nDecal],
                     olc::vf2d{1.0f, 1.0f} * BLOCK_SIZE, tint);
      }

    // draw mobs
    for (const auto &mob : vMobs) {
      olc::vi2d relSize = BLOCK_SIZE * olc::vi2d{1, 1};
      if (!tv.IsRectVisible(mob.pos * BLOCK_SIZE, relSize))
        continue;
      tv.DrawPartialDecal(mob.pos * BLOCK_SIZE, BLOCK_SIZE * relSize,
                          vWTDecals[(int)WT::MOBS][mob.nDecal], olc::vf2d{0, 0},
                          MOB_SIZE, mob.tint);
    }

    if (player.nDecal == 1) {
      // draw player
      tv.DrawPartialDecal(player.pos * BLOCK_SIZE,
                          vWTDecals[(int)WT::MOBS][player.nDecal],
                          olc::vf2d{0, 0}, MOB_SIZE,
                          BLOCK_SIZE * BLOCK_SIZE / MOB_SIZE, player.tint);
      // draw target
      tv.DrawDecal(target.pos * BLOCK_SIZE -
                       olc::vi2d{BLOCK_SIZE, BLOCK_SIZE} / 2,
                   target.decal, TARGET_SIZE * fBLOCK_SIZE);
    }

    // draw bullets
    for (const auto &b : vBullets) {
      tv.DrawRotatedDecal(b.pos * fBLOCK_SIZE, vBulletDecals[b.nDecal], b.fDir,
                          olc::vf2d{0.5f, 0.5f} * BULLET_SIZE,
                          olc::vf2d{1.0f, 1.0f} * BULLET_SIZE);
    }

    auto endTime = std::chrono::high_resolution_clock::now();
    float frameTime = std::chrono::duration_cast<std::chrono::milliseconds>(
                          endTime - startTime)
                          .count();
